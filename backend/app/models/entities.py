...
- `status` ('active' | 'completed' | 'abandoned') is what makes the
  brief's "Resume Session" feature possible — an 'active' session can be
  picked back up.
...
class User(Base):
    ...
    # Firebase UID — the stable identity from the frontend's existing auth.
...
class Session(Base):
    ...
    # 'active' | 'completed' | 'abandoned'  — drives Resume Session
...
class Persona(Base):
    ...
    # 'gentle' | 'balanced' | 'intense' — a coarse difficulty hint, optional.
